#!/usr/bin/env python3
#
# Creating the little Dirac operator
...
def fgcr(dim, blk_dim, vol, nmx, res, nkv, ncy, nmr, Dop, Mop, eta_ddtype_Re, eta_ddtype_Im, psi_ddtype_Re, psi_ddtype_Im, ddtype, sdtype, cdtype, xp, xp2, args):
    ...
    rn = complex_norm_sq(rho_sdtype_Re, rho_sdtype_Im, ddtype) # ddtype
    ...
    json_data['nmx'] = nmx
    json_data['nkv'] = nkv
    json_data['nmr'] = 0.5 if EXTENSION == 1 else nmr # 0.5 = adaptive
    json_data['ncy'] = 0.5 if EXTENSION == 1 else ncy # 0.5 = adaptive
    ...
    while rn > tol:
        ...
        for k in itertools.count(0): # k=0, 1, ..., infinite
            # start gcr_step()
            print('gcr_step() i={0}, status={1}, rn({2}), tol({3}), rn={4} > tol={5}'.format(i, status, type(rn), type(tol), rn, tol))

            json_data['i'].append(i)

            #print(type(phi_sdtype_Re[0,0]))
            ...
            if DIAGNOSTICS: print('b[{0}] = {1} ({2})'.format(k, b[k], type(b[k])))
            ...
            if DIAGNOSTICS: print(type(phi_sdtype_Re[k][0]), type(phi_sdtype_Re[k][0]), type(chi_sdtype_Re[k][0]), type(chi_sdtype_Re[k][0]))
            ...
            if rn <= tol or rn < get_precision_limit(ddtype)*rn_old or k+1 == nkv or status == nmx:
                ...
                if rn < (get_precision_limit(ddtype)*rn_old):
        ...
        rho_sdtype_Re, rho_sdtype_Im = axpy(-1.0, wrk_ddtype_Re, wrk_ddtype_Im, eta_ddtype_Re, eta_ddtype_Im, ddtype)
        rho_sdtype_Re, rho_sdtype_Im = xp.array(rho_sdtype_Re, dtype=sdtype), xp.array(rho_sdtype_Im, dtype=sdtype)

        rn = complex_norm_sq(rho_sdtype_Re, rho_sdtype_Im, ddtype)
        rn = np.sqrt(rn)
        print('update_psi() i={0}, status={1}, rn({2}), tol({3}), rn={4} > tol={5}'.format(i, status, type(rn), type(tol), rn, tol))
        json_data['rn'].append(np.float64(rn))
        json_data['i'].append(i)
...
def sap(i, vol, nmr, psi_Re, psi_Im, rho_Re, rho_Im, bs, Dop_blk, cdtype, xp, xp2, tol):
    ...
    for col, opposite_col in zip([0,1], [1,0]): # 0=black, 1=white
        ...
        for n in range(blk_start, blk_end):
            ...
            aimr[i] = imr
            i += 1
            if DIAGNOSTICS: print('blk_mres() rn={0}, n={1}'.format(np.sqrt(complex_norm_sq(blk_rho_Re, blk_rho_Im)), n))
...
def range_block(blk_dim, blk_vol, n):
    #start = blk_vol*TWELVE*n
    #e = blk_vol*TWELVE*(n+1)
    #end = e if e <= dim else dim
    start = blk_dim*n
    end   = blk_dim*(n+1)
...
def blk_mres(n, nmr, eta_Re, eta_Im, cdtype, sdtype, xp, tol):
    ...
    if EXTENSION == 1: rn = np.sqrt(complex_norm_sq(rho_Re, rho_Im, cdtype))
    #if EXTENSION == 1: print("---------------------")

    for imr in range(0, nmr):
        wrk_Re, wrk_Im = Dop_blk_sdtype(n, rho_Re, rho_Im)
        r = complex_norm_sq(wrk_Re, wrk_Im, cdtype)

        if DIAGNOSTICS: print("blk_mres() imr={0}, r={1}, {2}".format(imr, r, type(psi_Re)))

        if r < 2.0*get_min(sdtype): # overflow danger
            if DIAGNOSTICS: print("blk_mres() imr={0}, break".format(imr))
            break
        ...
        if EXTENSION == 1:
            ...
            if DIAGNOSTICS: print('mr_step() imr={0}, rn({1}), rn={2}'.format(imr, type(rn), rn))
            a = rn/rn_old
            #print('mr_step() imr={0}, rn={1}, alpha={2}'.format(imr, rn, a))
            #if rn >= rn_old:
            if (a >= alpha and imr+1 >= 4) or a >= 1 or rn <= tol:
                if DIAGNOSTICS:
                    if (a >= alpha and imr+1 >= 4): print('mr_step(): broke because of alpha >= {0} and imr+1 >= 3'.format(alpha))
...
def load_matrix_from_file(args, dtype = np.float64, sdtype = np.float64):
    ...
    if args.seven == False:
        ...
        if args.matfile != None:
            ...
            #kc = 0.15717 # critical value: 0 <= k < kc
            #k = 0.15717 # adjust this value from 0 to kc
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--vol', help='spacetime volume (vol=L1*L2*L3*L4).', type=int)
    parser.add_argument('-r', '--res', help='relative residue.', type=float)
    parser.add_argument('--nmx', help='maximal number of iterations for the outer GCR.', type=int, default=1)
    ...
    parser.add_argument('--ninv', help='number inverse iteration steps.', type=int, default=5)
    parser.add_argument('-Ns', help='number random vectors to start with.', type=int, default=20)
    ...
    parser.add_argument('-o', '--out', help='output file containing the little Dop.', default="-")
    ...
    # takes real amd imag parts and returns real and imaginary parts
    ...
    """
    def Dop_bnd_sdtype(color, re, im):
        return complex_matrix_vector_prod(Dop_bnd_sdtype_Re[color], Dop_bnd_sdtype_Im[color], re, im, sdtype)
    """
    ...
    if args.no_preconditioning == False:
        ...
        if args.no_reordering == False:
            ...
            # are associated to the 2 colors black and white. By defnition, even (black) indices come first,
        ...
        if args.no_reordering:
            ...
            Dop_black_int_bnd_sdtype_Re = bmat([[z, z], [Dop_ddtype_Re[dh:dim,0:dh], z]], format='csr', dtype=sdtype) # lower left block
            Dop_black_int_bnd_sdtype_Im = bmat([[z, z], [Dop_ddtype_Im[dh:dim,0:dh], z]], format='csr', dtype=sdtype)
            Dop_white_int_bnd_sdtype_Re = bmat([[z, Dop_ddtype_Re[0:dh,dh:dim]], [z, z]], format='csr', dtype=sdtype) # upper right block
        ...
        def Dop_int_color_bnd_sdtype(color, re, im):
            #return complex_matrix_vector_prod(Dop_color_int_bnd_sdtype_Re[color], Dop_color_int_bnd_sdtype_Im[color], re, im, sdtype)
            if color == 0:
    ...
    if args.no_preconditioning:
        def Mop(k, rho_Re, rho_Im, sdtype, dtype, tol):
            # Mop does no preconditioning
            phi_Re, phi_Im = xp.copy(rho_Re), xp.copy(rho_Im)
            chi_Re, chi_Im = Dop_sdtype(rho_Re, rho_Im)
            return phi_Re, phi_Im, chi_Re, chi_Im
    else:
        def Mop(k, rho_Re, rho_Im, sdtype, cdtype, tol):
            phi_Re = xp.zeros_like(rho_Re)
            phi_Im = xp.zeros_like(rho_Im)
            chi_Re = xp.copy(rho_Re)
            chi_Im = xp.copy(rho_Im)
            #if EXTENSION == 1: rn = np.sqrt(complex_norm_sq(chi_Re, chi_Im, cdtype))
            if EXTENSION == 1: rn = np.sqrt(complex_norm_sq(chi_Re, chi_Im, cdtype))
            ncy = args.ncy
            nmr = args.nmr
            #if EXTENSION == 1: ncy = int(np.floor(ncy/(2**k))+4) # min=1
            #if EXTENSION == 1: nmr = int(np.max([np.ceil(nmr - k*(nmr/3)), 4])) # min=4
            tnmr = 0
            for n in range(0, ncy):
                phi_Re, phi_Im, chi_Re, chi_Im, anmr = sap(n, vol, nmr, phi_Re, phi_Im, chi_Re, chi_Im, bs, Dop_blk_sdtype, cdtype, xp, xp2, tol)

                if EXTENSION == 1:
                    tnmr += anmr*nb
                    #rn_old = rn
                    #r_Re, r_Im = axpy(-1.0, chi_Re, chi_Im, rho_Re, rho_Im, sdtype)
                    #rn = np.sqrt(complex_norm_sq(r_Re, r_Im, cdtype))
                    rn_old = rn
                    rn = np.sqrt(complex_norm_sq(chi_Re, chi_Im, cdtype))
                    b = rn/rn_old
                    #print('sap() icy={0}, ncy={1}, rn={2} > tol={3}, beta={4}, nmr={5}'.format(n, ncy, rn, tol, b, nmr))
                    #print('sap() icy={0}, |phi|={2}'.format(n, ncy, rn2))
                    #print('sap() icy={0}, |chi|={2}'.format(n, ncy, rn3))
                    if (b >= max(beta, 1)) or b >= beta or rn < tol: break
                    #if rn >= rn_old: break
                ...
                if DIAGNOSTICS:
                    Dphi_Re, Dphi_Im = Dop_ddtype(phi_Re, phi_Im)
                    wrk_Re, wrk_Im = axpy(-1.0, Dphi_Re, Dphi_Im, rho_Re, rho_Im, sdtype)
                    rn_sap = np.sqrt(complex_norm_sq(wrk_Re, wrk_Im))
                    rn_chi = np.sqrt(complex_norm_sq(chi_Re, chi_Im))
                    wrk2_Re, wrk2_Im = axpy(-1.0, wrk_Re, wrk_Im, chi_Re, chi_Im, sdtype)
                    rn_next = np.sqrt(complex_norm_sq(wrk2_Re, wrk2_Im))
                    print('sap() n={0}, |Dphi - rho|={1}'.format(n, rn_sap))
                    print('sap() n={0}, |chi|={1}'.format(n, rn_chi))
                    print('sap() n={0}, sould be 0 |chi - (Dphi - rho)|={1}'.format(n, rn_next))

            #if EXTENSION == 1: and n != args.ncy-1: print('sap() icy={0}'.format(n))
            if EXTENSION == 1: print('sap() took ncy={0}/{1}, avg nmr={2}/{3}'.format(n+1, ncy, tnmr/(nb*(n+1)), nmr))
    ...
    bootstrap_Re = np.random.rand(args.Ns, dim)
    bootstrap_Im = np.random.rand(args.Ns, dim)
    bootstrap_rn = np.empty(args.Ns)
    M = np.empty(args.Ns)

    print("time", time.time(), time.time() - lastime); lastime = time.time();
    print("Calculating the Ns bootstrap fields...")
    for n in range(args.Ns):
        for i in range(args.ninv):
            bootstrap_Re[n], bootstrap_Im[n], bootstrap_rn[n], j = fgcr(dim, blk_dim, vol, args.nmx, 1e-12, args.nkv, args.ncy, args.nmr, Dop_ddtype, Mop, bootstrap_Re[n], bootstrap_Im[n], x0_ddtype_Re, x0_ddtype_Im, ddtype, sdtype, cdtype, xp, xp2, args)

        bn = np.sqrt(complex_norm_sq(bootstrap_Re[n], bootstrap_Im[n]))
        re1, im1 = bootstrap_Re[n]/bn, bootstrap_Im[n]/bn
        re2, im2 = Dop_ddtype(re1, im1)
        lam_Re, lam_Im = complex_scalar_prod(re1, im1, re2, im2)
        re3, im3 = complex_axpy(-lam_Re, -lam_Im, re1, im1, re2, im2)
        Dpn = np.sqrt(complex_norm_sq(re2, im2, ddtype))
        pn = np.sqrt(complex_norm_sq(re1, im1, ddtype))
        M[n] = Dpn/pn
        print("|D psi_l - lam psi_l | = {0}".format(np.sqrt(complex_norm_sq(re3, im3, ddtype))))

    print("|D psi_l| <= M |psi_l| for l = 1, ...,Ns. M={0}".format(np.max(M)))

    print("time", time.time(), time.time() - lastime); lastime = time.time();
    print("Generating the projected fields...")

    proj_bootstrap_Re = np.empty((nb*args.Ns, dim))
    proj_bootstrap_Im = np.empty((nb*args.Ns, dim))

    for b in range(0, nb):
        for i in range(0, args.Ns):
            bi = b*args.Ns + i
            proj_bootstrap_Re[bi] = projection(bootstrap_Re[i], b, blk_dim, blk_vol)
            proj_bootstrap_Im[bi] = projection(bootstrap_Im[i], b, blk_dim, blk_vol)

    print("Checking the projected fields...")
    for b in range(0, nb):
        for p in range(0, nb):
            if p != b:
                for i in range(0, args.Ns):
                    ...
                    for j in range(0, args.Ns):
                        pj = p*args.Ns + j
                        re, im = complex_scalar_prod(proj_bootstrap_Re[bi], proj_bootstrap_Im[bi], proj_bootstrap_Re[pj], proj_bootstrap_Im[pj])
                        if re != 0.0 or im != 0.0:
                            print("failed to project fields")
                            exit(1)

    print("Projected fields are in good condition.")

    print("time", time.time(), time.time() - lastime); lastime = time.time();
    print("Orthonormalising the projected fields on each block...")
    orth_Re = np.empty((nb*args.Ns, dim))
    orth_Im = np.empty((nb*args.Ns, dim))

    for b in range(0, nb):
        for i in range(0, args.Ns):
            bi = b*args.Ns + i
            orth_Re[bi] = proj_bootstrap_Re[bi]
            orth_Im[bi] = proj_bootstrap_Im[bi]
            for k in range(0, i):
                bk = b*args.Ns + k
                beta_ik_Re, beta_ik_Im = complex_scalar_prod(orth_Re[bk], orth_Im[bk], proj_bootstrap_Re[bi], proj_bootstrap_Im[bi])
                orth_Re[bi], orth_Im[bi] = complex_axpy(-beta_ik_Re, -beta_ik_Im, orth_Re[bk], orth_Im[bk], orth_Re[bi], orth_Im[bi], sdtype)
            norm = np.sqrt(complex_norm_sq(orth_Re[bi], orth_Im[bi]))
            orth_Re[bi] = orth_Re[bi]/norm
            orth_Im[bi] = orth_Im[bi]/norm

    print("Checking the ortonormalised projected fields...")
    for b in range(0, nb):
        for p in range(0, nb):
            for i in range(0, args.Ns):
                bi = b*args.Ns + i
                for j in range(0, args.Ns):
                    pj = p*args.Ns + j
                    re, im = complex_scalar_prod(orth_Re[bi], orth_Im[bi], orth_Re[pj], orth_Im[pj])
                    if b == p and i == j: re -= 1.0
                    if not np.allclose([re, im], [0.0, 0.0]):
                        print(re, im)
                        print("fields are not orthonormal")
                        exit(1)

    print("Projected fields are mutually orthonormal.")
    del proj_bootstrap_Re, proj_bootstrap_Im, bootstrap_Re, bootstrap_Im, bootstrap_rn # cleaning up memory

    print("time", time.time(), time.time() - lastime); lastime = time.time();
    print("Constructing the little Dop...")
    A_Re = np.zeros((nb*args.Ns, nb*args.Ns))
    A_Im = np.zeros((nb*args.Ns, nb*args.Ns))
    for l in range(nb*args.Ns):
        re, im = Dop_ddtype(orth_Re[l], orth_Im[l])
        for k in range(nb*args.Ns):
            A_Re[k,l], A_Im[k,l] = complex_scalar_prod(orth_Re[k], orth_Im[k], re, im)

    print("time", time.time(), time.time() - lastime); lastime = time.time();
    print("Storing little Dop in outfile...")
    with open(args.out, 'wb') as out:
        fmt = '<{0}d'
        out.write(struct.pack('<1I', args.Ns))
        out.write(struct.pack('<1I', nb))
        for l in range(nb*args.Ns):
            out.write(struct.pack(fmt.format(nb*args.Ns), *A_Re[l]))
        for l in range(nb*args.Ns):
            out.write(struct.pack(fmt.format(nb*args.Ns), *A_Im[l]))
        for l in range(nb*args.Ns):
            out.write(struct.pack(fmt.format(dim), *orth_Re[l]))
        for l in range(nb*args.Ns):
            out.write(struct.pack(fmt.format(dim), *orth_Im[l]))
