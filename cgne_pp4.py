...
# without diagnostics
def cgne_adv_DdaggerD(vol, nmx, res, Dop_Re, Dop_Im, Dop_dble_Re, Dop_dble_Im, eta_Re, eta_Im, psi_Re, psi_Im, sdtype, ddtype, cdtype, extension = 0):
    ...
    while rn > tol or status == 0:
        ...
        while True:
            ...
            psap_Re, psap_Im = complex_matrix_vector_prod(Dop_Re, Dop_Im, psp_Re, psp_Im, sdtype)

            ai = rsq/complex_scalar_prod(psp_Re, psp_Im, psap_Re, psap_Im, cdtype)[0] # cdtype
        ...
        pdv_Re, pdv_Im = complex_matrix_vector_prod(Dop_dble_Re, Dop_dble_Im, pdx_Re, pdx_Im, ddtype)
        ...
        psap_Re, psap_Im = complex_matrix_vector_prod(Dop_Re, Dop_Im, psw_Re, psw_Im, sdtype)

        # r = |D psw|^2 = psw^^dagger D^dagger D psw = psw^dagger psap
        r = complex_scalar_prod(psw_Re, psw_Im, psap_Re, psap_Im, cdtype)[0] # cdtype
...
parser.add_argument('file', help='the input file containing the parameters and D^dagger D as ONE operator in CSR format.')
...
with open(args.file, "rb") as f:
    ...
    print("retrieving D^dagger D matrix in csr format (Re,dble) ...")
    ...
    A_dble_Re = csr_matrix([data, index, ptr], ddtype)
    A_dble_Re.tr_rdata, A_dble_Re.tr_col_index, A_dble_Re.tr_row_ptr = tr_data, tr_index, tr_ptr
    A_dble_Re.tr_calculated = True
    ...
    A_Re = csr_matrix([data2, index, ptr], dtype)
    A_Re.tr_rdata, A_Re.tr_col_index, A_Re.tr_row_ptr = tr_data2, tr_index, tr_ptr
    ...
    # creating the object
    A_dble_Im = csr_matrix([data, index, ptr], ddtype)
    A_dble_Im.tr_rdata, A_dble_Im.tr_col_index, A_dble_Im.tr_row_ptr = tr_data, tr_index, tr_ptr
    A_dble_Im.tr_calculated = True
    ...
    data2 = np.array([dtype(x) for x in data])
    tr_data2 = np.array([dtype(x) for x in tr_data])
    A_Im = csr_matrix([data2, index, ptr], dtype)
    A_Im.tr_rdata, A_Im.tr_col_index, A_Im.tr_row_ptr = tr_data2, tr_index, tr_ptr
    ...
    #print(A_Re.asarray())
    #print(Dop_Re.shape)
    #print(A_Re.asarray().shape)
    #print(np.array_equal(Dop_Re.transpose(), A_dble_Re.transpose().asarray()))
    #print(np.array_equal(Dop_Re, A_Re.asarray()))
    #print(np.array_equal(Dop_Re, A_Re.transpose().transpose().asarray()))
    #print(np.array_equal(A_Re.asarray(), A_Re.transpose().transpose().asarray()))
    #exit()
    ...
    print("calculating r = D^dagger D b (1/2)...")
    r_Re, r_Im = complex_matrix_vector_prod(A_Re, A_Im, b_Re, b_Im, dtype)
    print(type(r_Re), type(r_Re[0]))
    print("time", time.time(), time.time() - lastime); lastime = time.time();

    print("calculating r = D^dagger D r (2/2)...")
    r_Re, r_Im = complex_matrix_vector_prod(A_Re, A_Im, r_Re, r_Im, dtype)
    print(type(r_Re), type(r_Re[0]))
    print("time", time.time(), time.time() - lastime); lastime = time.time();

    print("calculating Ax=b ...")
    x_Re, x_Im, rn, json_data = cgne_adv_DdaggerD(vol, nmx, res, A_Re, A_Im, A_dble_Re, A_dble_Im, b_dble_Re, b_dble_Im, x0_dble_Re, x0_dble_Im, dtype, ddtype, cdtype, args.extension)
    ...
    # ---------------------------------OLD-----------------------------------------------
    #print("calculating Ax=b ...")
    #x_Re, x_Im, rn, json_data = cgne(vol, nmx, res, A_Re, A_Im, b_Re, b_Im, x0_Re, x0_Im, dtype)
    #print("rn=", rn, type(x_Re[0]), type(x_Re[0]))
    #print("status", json_data['status'])
    #print("time", time.time(), time.time() - lastime); lastime = time.time();

    print("calculating |A_dble x - b| ...")
    res = Axmb(A_dble_Re, A_dble_Im, x_Re, x_Im, b_dble_Re, b_dble_Im, ddtype)
